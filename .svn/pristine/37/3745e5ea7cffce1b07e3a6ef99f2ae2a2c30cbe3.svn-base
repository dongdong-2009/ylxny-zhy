$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '禁用时间', name: 'modifyTime', index: 'modify_time', width: 80 }, 			
			{ label: '', name: 'companyId', index: 'company_id', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.user.userId == null ? "qxgl/user/save" : "qxgl/user/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		getInfo: function(userId){
			$.get(baseURL + "qxgl/user/info/"+userId, function(r){
                vm.user = r.user;
            });
		},
