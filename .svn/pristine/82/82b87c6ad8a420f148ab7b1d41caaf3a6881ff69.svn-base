...
public class SummaryDayServiceImpl extends ServiceImpl<SummaryDayDao, SummaryDayEntity> implements SummaryDayService {
	...
	@Override
	public JSONObject getDevideFee(String date, Long companyId) {
	...
	@Override
	public List<FuheReportEntity> getFuheDayList(Map<String, Object> params, Long companyId) {
		Long stationId = Long.parseLong(CommonUtil.getString(params.get("stationId")));
		Long lineId = Long.parseLong(CommonUtil.getString(params.get("lineId")));
		String date = CommonUtil.getString(params.get("date"));
		EntityWrapper<SummaryDayEntity> w = new EntityWrapper<SummaryDayEntity>();
		w.eq("company_id", companyId);
		w.eq("station_id", stationId);
		w.eq("line_id", lineId);
		w.like("date", date);
		Page<SummaryDayEntity> page = summaryDayService.selectPage(new Query<SummaryDayEntity>(params).getPage(),w);
		List<FuheReportEntity> reports = new ArrayList<FuheReportEntity>();
		for(Iterator<SummaryDayEntity> it = page.getRecords().iterator(); it.hasNext();){
			SummaryDayEntity entity = it.next();
